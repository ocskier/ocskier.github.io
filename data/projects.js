...
$.each(shuffleArray(projectsArray), function (index, project) {
  $('#portfolio-content').append(`
    <li class="full-thumb projectLi ${project.tech
      .map((tech) => `f-${tech}`)
      .join(' ')}">
        ...
        <div style="display: inline-flex;">
          <a href=${
            project.url ? project.url : project.ghLink
          } class="fancybox" rel="group" title=${project.name} target="_blank">
            <div class="thumb-icon-photo"></div>
          </a>
          <div class="thumb-icon-github">
            <a id="logo-container" href=${
              project.ghLink
            } class="brand-logo" target="_blank">
